...
static struct taskqueue default_queue = {"bycorf worker"};
...
bool bycorf_bsd_tasks_pending(void) { return head != NULL; }
...
int taskqueue_start_threads(struct taskqueue** q, int count, int priority,
                            const char* name, ...) {
  // This queue is serviced by bycorf_bsd_poll on its owning worker. No kernel
...
void taskqueue_drain(struct taskqueue* q, struct task* task) {
  (void)q;
  while (task->ta_pending) bycorf_bsd_tasks_poll();
}
